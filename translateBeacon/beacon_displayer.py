...
root = Tk()
topFrame = Frame(root, relief = GROOVE)
topFrame.pack()
...
zuluTime.pack(side = TOP)
zuluTime = Label(root, relief = GROOVE, text = zuluHour + ":" + zuluMinute + ":" + zuluSecond)
zuluTime.pack(side = TOP)

leftFrame = Frame(root, relief = GROOVE)
leftFrame.pack(side = LEFT)

rightFrame = Frame(root, relief = GROOVE)
...
# def beacon_byte_calculator_no_print ():
...
#
# def beacon_byte_calculator_print():
#   number_of_elements = len(datatype)
#   i = 0
#   start_byte = 0
#   while (i < number_of_elements):
